...
export const NavBar = () => {
  ...
  const [auth, setAuth] =
    useState<ReturnType<typeof authStorage.getAuthSession>>(null);

  // init auth session, and fix hydration mismatch
  useEffect(() => {
    function init() {
  ...
  return (
    <nav className="flex w-full items-center justify-between bg-white px-6 py-4 shadow-md">
      ...
      <div className="flex items-center gap-4">
        <span className="text-gray-700">Hi, {auth?.user?.full_name}</span>
        <Button
